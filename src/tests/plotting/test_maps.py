...
def test_plot_global_map_no_features():
    plot = CreatePlot(); plot.projection = "plcarr"; plot.domain = "global"
...
def test_plot_global_map_coastlines_and_labels():
    ...
    plot.add_xlabel(xlabel="longitude"); plot.add_ylabel(ylabel="latitude")
    fig = CreateFigure(); fig.plot_list = [plot]; fig.create_figure()
    ax = fig.fig.axes[0]
...
def test_plot_map_scatter_conus_with_colorbar():
    ...
    scatter.cmap = "Blues"; scatter.markersize = 25
    plot = CreatePlot(plot_layers=[scatter]); plot.projection = "plcarr"; plot.domain = "conus"
...
def test_plot_map_scatter_2d_no_colorbar():
    scatter = MapScatter(latitude=np.linspace(35, 50, 30),
                         longitude=np.linspace(-70, -120, 30))
    scatter.color = "tab:red"; scatter.markersize = 25
    plot = CreatePlot(plot_layers=[scatter]); plot.projection = "plcarr"; plot.domain = "conus"
    plot.add_map_features(["coastline", "states"])
    fig = CreateFigure(); fig.plot_list = [plot]; fig.create_figure()
...
def test_plot_map_gridded_global():
    lats = np.linspace(25, 50, 25); lons = np.linspace(245, 290, 45)
    X, Y = np.meshgrid(lons, lats); Z = np.random.normal(size=X.shape)
    gridded = MapGridded(X, Y, Z)
    plot = CreatePlot(plot_layers=[gridded]); plot.projection = "plcarr"; plot.domain = "global"
...
def test_plot_map_contour_global_combo():
    x, y, z = _contour_data((20, 40)); z = z * -1.5 * x
    contour = MapContour(x, y, z); gridded = MapGridded(x, y, z)
    plot = CreatePlot(plot_layers=[contour, gridded]); plot.projection = "plcarr"; plot.domain = "global"
    plot.add_map_features(["coastline"])
    fig = CreateFigure(); fig.plot_list = [plot]; fig.create_figure()
    ax = fig.fig.axes[0]
...
def test_plot_map_filled_contour_global():
    x, y, z = _contour_data((20, 40)); z = z * -1.5 * x
    contourf = MapFilledContour(x, y, z); contourf.cmap = "viridis"
    contour = MapContour(x, y, z)
    plot = CreatePlot(plot_layers=[contourf, contour]); plot.projection = "plcarr"; plot.domain = "global"
    plot.add_map_features(["coastline"])
    fig = CreateFigure(); fig.plot_list = [plot]; fig.create_figure()
    ax = fig.fig.axes[0]
    assert len(ax.collections) >= 2
...
def test_plot_map_multidata_conus_with_colorbar():
    lats = np.linspace(25, 50, 25); lons = np.linspace(245, 290, 45)
    X, Y = np.meshgrid(lons, lats); Z = np.random.normal(size=X.shape)
    gridded = MapGridded(X, Y, Z); scatter = MapScatter(
        latitude=np.linspace(35, 50, 30),
        longitude=np.linspace(-70, -120, 30),
        data=np.linspace(200, 300, 30),
    )
    scatter.cmap = "Reds"; scatter.markersize = 100; scatter.colorbar = False
    plot = CreatePlot(plot_layers=[gridded, scatter]); plot.projection = "plc_
